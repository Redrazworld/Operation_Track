...
def payment_and_pending():
    ...
    if emp_id:
        try:
            ...
            # Filter the DataFrame based on the selected date range and employee ID
            ...
            # Calculate the pending amount for each order
            filtered_df["Pending Amount"] = filtered_df["Total Price"] - filtered_df["Amount Received"]

            if not filtered_df.empty:
                st.write("Data with Pending Amount:")
                st.write(filtered_df[filtered_df["Pending Amount"] > 0])
            else:
